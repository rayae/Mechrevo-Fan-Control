...
func registerSystray() {
	if err := wt.initInstance(); err != nil {
		panic(fmt.Errorf("%v: %v", err, "cannot initInstance"))
	}

	if err := wt.createMenu(); err != nil {
		panic(fmt.Errorf("%v: %v", err, "cannot createMenu"))
	}
...
func nativeLoop() {
	...
	for {
		...
		switch int32(ret) {
		case -1:
			zap.S().Errorf("Error at message loop: %v", err)
			return
		case 0:
			return
		default:
...
func SetIcon(iconBytes []byte) {
	...
	if err != nil {
		zap.S().Errorf("Unable to write icon data to temp file: %v", err)
		return
	}
	if err := wt.setIcon(iconFilePath); err != nil {
		zap.S().Errorf("Unable to set icon: %v", err)
		return
	}
}
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	...
	if err != nil {
		zap.S().Errorf("Unable to load icon from temp file: %v", err)
		return
	}

	h, err = wt.iconToBitmap(h)
	if err != nil {
		zap.S().Errorf("Unable to convert icon to bitmap: %v", err)
		return
	}
	...
	if err != nil {
		zap.S().Errorf("Unable to addOrUpdateMenuItem: %v", err)
		return
	}
}
...
func SetTooltip(tooltip string) {
	if err := wt.setTooltip(tooltip); err != nil {
		zap.S().Errorf("Unable to set tooltip: %v", err)
		return
	}
}
...
func addSeparator(id uint32) {
	...
	if err != nil {
		zap.S().Errorf("Unable to addSeparator: %v", err)
		return
	}
}
...
func hideMenuItem(item *MenuItem) {
	...
	if err != nil {
		zap.S().Errorf("Unable to hideMenuItem: %v", err)
		return
	}
}
